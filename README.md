...
- Agent 的預設自動重連間隔現在也已調整為 `60` 秒；若 WebSocket 中斷，Agent 預設每分鐘重試一次，避免大量 Agent 同時掉線時每 `5` 秒持續重試造成集中 reconnect burst。
...
- Agent 已改為 `net48`，並在同一個 `deploy/publish/Agent` 目錄同時輸出 64 位元與 32 位元版本，原因是 `net8.0-windows` 與後續回退的 `net6.0-windows` 在部分 Windows 7 仍會於啟動前命中 `KERNEL32.dll` 缺少 `LocateXStateFeature` / `GetEnabledXStateFeatures`；改回 `.NET Framework 4.8` 後可避開這類原生 runtime 載入錯誤。Host / Server 仍維持 `net8`，只有 Agent 針對 Win7 相容性採用不同執行框架。
...
- 若中央模式下的 `RemoteDesktop.Server` 暫時未啟動或無法連線，Host 設定視窗現在仍可開啟；系統會自動 fallback 讀取本機 `appsettings.json`，並提示目前儲存只會更新本機設定、不會同步到中央 Server，方便先修正 `CentralServerUrl` 或切回本機模式。
- 中央模式下的 `RemoteDesktop.Host` 現在改為「開啟主控台時先向中央 Server 抓一次資料，之後只在按下重新整理時才再次抓取」；不再維持 `/ws/dashboard` 即時推播連線，也不再做背景輪詢，降低 Host 長時間開著時對中央 Server 的管理面封包量。
...
- `Deploy-App.ps1` 現在會將 Host / Server 發布為單檔 self-contained EXE；Agent 則會在同一個 `deploy/publish/Agent` 目錄輸出 `RemoteDesktop.Agent.exe` 與 `RemoteDesktop.Agent.x86.exe`，並附帶相依 `.dll` / `.config` 檔，以保留 Windows 7 相容性與現場設定彈性。
...
- Host / Server 的 publish 版仍是單檔自帶 runtime；Agent 則需連同同目錄 DLL 一起部署，目標框架為 `.NET Framework 4.8`。
- 若 Win7 機器尚未安裝 `.NET Framework 4.8`，需先安裝後再執行 Agent。
- Agent 的 publish 版目前是為了 Win7 相容性改成 `net48`，並同時提供 64 位元與 32 位元兩支 EXE；若現場只跑 Windows 10/11 或 Windows Server，仍可和 `net8` 的 Host / Server 正常互通。
