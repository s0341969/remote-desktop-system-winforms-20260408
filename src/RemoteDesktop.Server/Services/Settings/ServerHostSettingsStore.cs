...
public interface IServerHostSettingsStore
{
    Task<HostSettingsDto> LoadAsync(CancellationToken cancellationToken);

    Task SaveAsync(HostSettingsDto document, CancellationToken cancellationToken);
}

public sealed class ServerHostSettingsStore : IServerHostSettingsStore
{
    ...
    public ServerHostSettingsStore(IHostEnvironment environment, IConfiguration configuration, IOptions<ControlServerOptions> options)
    {
    ...
    {
        var document = new HostSettingsDto
        {
        ...
        {
            document = new HostSettingsDto
            {
                EnableDatabase = document.EnableDatabase,
                RemoteDesktopDbConnectionString = connectionString,
        ...
        if (controlServer is not null)
        {
            document = new HostSettingsDto
            {
                ...
                ServerUrl = controlServer["ServerUrl"]?.GetValue<string>() ?? document.ServerUrl,
                ConsoleName = controlServer["ConsoleName"]?.GetValue<string>() ?? document.ConsoleName,
                AdminUserName = controlServer["AdminUserName"]?.GetValue<string>() ?? document.AdminUserName,
                AdminPassword = controlServer["AdminPassword"]?.GetValue<string>() ?? document.AdminPassword,
                SharedAccessKey = controlServer["SharedAccessKey"]?.GetValue<string>() ?? document.SharedAccessKey,
                RequireHttpsRedirect = controlServer["RequireHttpsRedirect"]?.GetValue<bool?>() ?? document.RequireHttpsRedirect,
                AgentHeartbeatTimeoutSeconds = controlServer["AgentHeartbeatTimeoutSeconds"]?.GetValue<int?>() ?? document.AgentHeartbeatTimeoutSeconds
            };
        }
    ...
    public async Task SaveAsync(HostSettingsDto document, CancellationToken cancellationToken)
    {
        Validate(document);
